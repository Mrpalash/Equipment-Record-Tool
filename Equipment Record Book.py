...
root = Tk()
root.title("Equipment Digital Record Book")
...
def Exit():
    result = tkMessageBox.askquestion('Equipment Digital Record Book', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("Equipment Digital Record Book/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    global Home
    Home = Tk()
    Home.title("Equipment Digital Record Book/Home")
    width = 1024
    height = 520
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="Equipment Digital Record Book", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
    ...
    menubar.add_cascade(label="Records", menu=filemenu2)
...
def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("Equipment Digital Record Book/Add new")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Delete():
    if not tree.selection():
        ...
        result = tkMessageBox.askquestion('Equipment Digital Record Book', 'Are you sure you want to delete this record?',
                                          icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("Equipment Digital Record Book/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('Equipment Digital Record Book', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes':
        admin_id = ""
        root.deiconify()
        Home.destroy()
